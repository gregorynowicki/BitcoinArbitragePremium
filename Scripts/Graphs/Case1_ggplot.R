...
returns <- ggplot(data=case1_1d,aes(Date,Return))+
  geom_line(aes(case1_1d$Date,case1_1d$Return),color='red') +
  geom_line(aes(case1_2d$Date,case1_2d$Return),color='blue')+
  geom_line(aes(case1_3d$Date,case1_3d$Return),color='black')+
  geom_hline(yintercept = c(0))+
  geom_vline(xintercept = as.numeric(as.Date('2014-03-01')))+
  scale_x_date(limits = as.Date(c('2014-03-01','2017-03-02')),expand=c(0,0))+
  scale_y_continuous(limits = c(0,1),expand = c(0,0),labels = percent,breaks=c(.25,.5,.75,1))+
  ...
  theme(panel.background = element_rect(fill = "white",
        ...
        panel.grid.major.y = element_line(size = 0.5, linetype = 'solid',
                                          colour = "grey")) +
  ggtitle("Case 1: Monthly Returns From March 2014") +
...
ggsave("Case1_MonthlyReturn_P2.pdf", returns, width=10, height=6)
