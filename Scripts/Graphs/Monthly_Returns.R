...
spreads <- ggplot()+
  geom_line(aes(case1_stats$Date,case1_stats$WeightedAverageSpread),color='red') +
  geom_line(aes(case2_stats$Date,case2_stats$WeightedAverageSpread),color='blue') +
  geom_hline(yintercept = c(0))+
  geom_vline(xintercept = as.numeric(as.Date('2010-07-01')))+
  scale_x_date(limits = as.Date(c('2010-07-01','2017-03-02')),expand=c(0,0.5))+
  scale_y_continuous(limits = c(0,100),expand = c(0,0.75))+
  ...
  theme(plot.title = element_text(family = "CM Roman", color="#666666", size=22, hjust=.5)) +
  theme(axis.title = element_text(family = "CM Roman", color="#666666", face="bold", size=18))
  
ggsave("Monthly_WgtAvgSpread.pdf", spreads, width=10, height=6)
